#!/usr/bin/env python3
"""
ClearCouncil - Enhanced Production Server
...
class DocumentProcessor:
    ...
    def process_pdf_document(self, file_path: Path, document_type: str = 'meeting_minutes') -> Dict:
        """Process PDF documents (meeting minutes, agendas, etc.)"""
        try:
    ...
    def extract_pdf_text(self, file_path: Path) -> str:
        ...
        try:
            ...
            try:
                ...
                return f"Document uploaded: {file_path.name} (text extraction not available)"
...
class EnhancedDataAccess:
    """Enhanced data access for chat integration"""
    ...
    def get_voting_records(self, topic: str = None, representative: str = None) -> List[Dict]:
        ...
        with self.get_connection() as conn:
            query = 'SELECT * FROM voting_records WHERE 1=1'
            params = []
            
            if topic:
                query += ' AND (bill_title LIKE ? OR description LIKE ?)'
...
• {len(meetings)} Recent meetings available
• Voting records and analysis tools
...
What would you like to explore?"""
        ...
        if any(word in message_lower for word in ['representative', 'rep', 'council member', 'who represents']):
            ...
            if reps:
                rep_list = []
                for rep in reps[:5]:  # Show top 5
                    rep_info = f"**{rep['name']}** - {rep['position']}"
                    if rep['district']:
...
💡 *Ask me specific questions like "How did [name] vote?" or "Tell me about [name]"*"""
            else:
...
💡 *Ask me to "search for [topic] in meetings" for specific information*"""
            else:
                return "❌ No meeting data is currently available. You can upload meeting minutes using the document upload feature."
        ...
        if any(word in message_lower for word in ['vote', 'voting', 'ballot', 'voted']):
            ...
            if voting_records:
                ...
                for vote in voting_records[:5]:
                    vote_info = f"📊 **{vote.get('bill_title', 'Unknown Bill')}**"
                    vote_info += f"\n   👤 {vote.get('representative_name', 'Unknown')}: {vote.get('vote', 'Unknown')}"
                    if vote.get('date'):
                        vote_info += f"\n   📅 {vote['date']}"
...
💡 *Be more specific: "How did John Smith vote on budget?" or "Show votes on healthcare"*"""
            else:
                return "❌ No voting records found for your query. Try a different representative name or topic."
...
💡 *Ask for more specific information about any document*"""
                else:
                    return f"❌ No documents found containing '{' '.join(search_terms)}'. Try different search terms."
            else:
...
🤔 I'm not sure how to help with that specific question. Here are some things I can do:

• **"Show representatives"** - List your council members
• **"Recent meetings"** - Show latest meeting minutes  
• **"Search for [topic]"** - Find relevant documents
• **"Voting records"** - Show how representatives voted

💡 Try rephrasing your question or ask "help" for more options."""
    ...
    def extract_topic(self, message: str) -> Optional[str]:
        ...
        # Extract words after "on" or "about"
        message_lower = message.lower()
...
<head>
    ...
    <style>
        ...
        .bot-message .content { background: #f1f1f1; padding: 10px 15px; border-radius: 15px; display: inline-block; max-width: 80%; }
            ...
            <div class="feature-card">
                ...
                <div class="chat-container">
                    <div class="chat-header">
                        <strong>ClearCouncil Assistant</strong>
                    </div>
                    <div class="chat-messages" id="chat-messages">
                        <div class="message bot-message">
                            <div class="content">Hello! I can help you explore government data. Try asking about representatives, meetings, or voting records.</div>
                        </div>
                    </div>
...
def upload_file():
    ...
    if file:
        ...
        result = doc_processor.process_pdf_document(file_path, 'meeting_minutes')
...
if __name__ == '__main__':
    print("🚀 ClearCouncil Enhanced Server Starting...")
    print("=" * 60)
