...
from clearcouncil_next.core.privacy import redact_object
...
def run_ingest_job(council_key: str, payload: dict) -> None:
    ...
    logger.info("Running ingest job", extra={"council_key": council_key, "payload": redact_object(payload)})
    time.sleep(0.25)


def run_reindex_job(council_key: str, payload: dict) -> None:
    """Placeholder for vector and metadata reindex."""
    logger.info("Running reindex job", extra={"council_key": council_key, "payload": redact_object(payload)})
    time.sleep(0.25)


def run_eval_job(council_key: str, payload: dict) -> None:
    """Placeholder for extraction and grounding evaluation jobs."""
    logger.info("Running eval job", extra={"council_key": council_key, "payload": redact_object(payload)})
    time.sleep(0.25)
